...
@section('title', 'Edit Kepala Sekolah - SMKN 2 SIMPANG EMPAT Tanah Bumbu')

@section('topbar')
<div class="bg-white shadow-md px-8 py-4">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Edit Kepala Sekolah</h2>
            <p class="text-gray-600">Edit data kepala sekolah: {{ $kepalaSekolah->nama }}</p>
        </div>
        ...
        <form action="{{ route('admin.kepala-sekolah.update', $kepalaSekolah) }}" method="POST">
            ...
            <div class="mb-6">
                ...
                <input type="text" 
                    id="nip" 
                    name="nip" 
                    value="{{ old('nip', $kepalaSekolah->nip) }}"
            ...
            <div class="mb-6">
                ...
                <input type="text" 
                    id="pangkat" 
                    name="pangkat" 
                    value="{{ old('pangkat', $kepalaSekolah->pangkat) }}"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    placeholder="Contoh: Pembina Utama, Pembina, Penata, dll."
                    required>
            </div>

            <!-- Tanggal Menjabat -->
            <div class="mb-6">
                <label for="tanggal_menjabat" class="block text-sm font-semibold text-gray-700 mb-2">Tanggal Menjabat <span class="text-red-500">*</span></label>
                <input type="date" 
                    id="tanggal_menjabat" 
                    name="tanggal_menjabat" 
                    value="{{ old('tanggal_menjabat', $kepalaSekolah->tanggal_menjabat ? $kepalaSekolah->tanggal_menjabat->format('Y-m-d') : '') }}"
                    class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    required>
            </div>
