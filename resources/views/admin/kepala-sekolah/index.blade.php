...
@section('title', 'Data Kepala Sekolah - SMKN 2 SIMPANG EMPAT Tanah Bumbu')
...
<div class="bg-white shadow-md px-8 py-4">
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600">Kelola data kepala sekolah SMKN 2 SIMPANG EMPAT</p>
        </div>
...
<div class="bg-white rounded-xl shadow-lg p-6 mb-6">
    <form action="{{ route('admin.kepala-sekolah.index') }}" method="GET">
        <div class="flex items-center space-x-4">
        ...
        <table class="w-full">
            <thead class="bg-gray-50 border-b border-gray-200">
                <tr>
                    ...
                    <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Tanggal Menjabat</th>
                    <th class="px-6 py-4 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">Aksi</th>
                </tr>
            </thead>
            <tbody class="divide-y divide-gray-200">
                @forelse($kepalaSekolahs as $kepalaSekolah)
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                        {{ ($kepalaSekolahs->currentPage() - 1) * $kepalaSekolahs->perPage() + $loop->index + 1 }}
                    </td>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                        {{ $kepalaSekolah->pangkat }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                        {{ $kepalaSekolah->tanggal_menjabat ? $kepalaSekolah->tanggal_menjabat->format('d/m/Y') : '-' }}
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                        <div class="flex items-center justify-center space-x-2">
                            <a href="{{ route('admin.kepala-sekolah.edit', $kepalaSekolah) }}" 
                            ...
                            <form action="{{ route('admin.kepala-sekolah.destroy', $kepalaSekolah) }}" 
                                method="POST" 
                                onsubmit="return confirm('Apakah Anda yakin ingin menghapus data kepala sekolah ini?')"
                                class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" 
                ...
                <tr>
                    <td colspan="6" class="px-6 py-12 text-center">
                        <div class="flex flex-col items-center justify-center">
                            ...
                            <p class="text-gray-500 text-lg">Tidak ada data kepala sekolah ditemukan</p>
                            <p class="text-gray-400 text-sm mt-1">Silakan tambah data kepala sekolah baru</p>
                        </div>
                    </td>
                </tr>
                @endforelse
