...
@section('title', 'Prestasi Siswa - SMKN 2 SIMPANG EMPAT Tanah Bumbu')

@section('topbar')
<div class="bg-white shadow-md px-8 py-4">
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800">Prestasi Siswa</h2>
            <p class="text-gray-600">Kelola data prestasi siswa</p>
        </div>
        <div class="flex items-center space-x-4">
            <a href="{{ route('admin.prestasi-siswa.create') }}" 
...
<div class="space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-lg p-6">
        <form action="{{ route('admin.prestasi-siswa.index') }}" method="GET" class="flex items-center space-x-4">
            <div class="flex-1">
                <input type="text" 
                    name="search" 
                    value="{{ $search ?? '' }}"
                    placeholder="Cari berdasarkan nomor, nama siswa, NIS, atau kepala sekolah..."
    ...
    <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <table class="w-full">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Kepala Sekolah</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                            {{ ($prestasis->currentPage() - 1) * $prestasis->perPage() + $index + 1 }}
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                            <div>
                                <p class="font-medium text-gray-900">{{ $prestasi->siswa->nama }}</p>
                                <p class="text-sm text-gray-500">{{ $prestasi->siswa->nis }}</p>
                            </div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                            {{ $prestasi->kepalaSekolah->nama }}
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                            <div class="flex items-center space-x-2">
                                <a href="{{ route('admin.prestasi-siswa.edit', $prestasi) }}" 
                                ...
                                <form action="{{ route('admin.prestasi-siswa.destroy', $prestasi) }}" method="POST" 
                    ...
                    <tr>
                        <td colspan="7" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <p class="text-gray-500 text-lg">Tidak ada data prestasi siswa ditemukan</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
        ...
        <div class="px-6 py-4 bg-gray-50 border-t border-gray-200">
            {{ $prestasis->appends(['search' => $search])->links() }}
        </div>
        @endif
    </div>
</div>
